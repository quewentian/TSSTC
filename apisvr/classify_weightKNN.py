...
#function：读取五大类别的关键词
def readCategoryKeyWord():
    cateWordDict={}##{每个类别：类别对应的keyword关键词的list[]}
    cateWeightDict={}
    dir='BBC-train'
    ...
    for i in range(len(dir_list)):
        ...
        # print(cate)
        cate_keyword_list,cate_weight_list=readFile(path)
        cateWordDict[cate]=cate_keyword_list
        cateWeightDict[cate]=cate_weight_list
    # print(cateDict)
    return cateWordDict,cateWeightDict
...
##计算欧式距离
def ouDistance(vector1,vector2,weight1,weight2):
    ou = np.sqrt(weight1*weight2*np.sum(np.square(vector1 - vector2)))
    return ou

###实在不行搞个投票机制
def categorize_knn_weight(category,vec,category_weight_dict,predictFile_weight_list):
    # distDict={}###距离词典{‘距离大小’，类别}，待会按照距离大小排序，最后取前十的items
    # tenCate=[]
    cate_weight={}
    # softmax(category_weight_dict,1)
    for key in category_weight_dict:
        category_weight_dict[key]=softmax(category_weight_dict[key],1)

    predict_weight=softmax(predictFile_weight_list,1)##list
    ...
    for n in range(len(vec)):########每个词10个票。（之后挑出1个最可能的类）
        ...
        for key in category:
            ...
            # for n in range(len(vec)):
                dist=ouDistance(category[key][i],vec[n],category_weight_dict[key][i],predict_weight[n])
                ...
                distDict[dist] = key  ##对于每一个词：{dist1:class1,dist2:class2,...dist10:class3}
...
def classify(test_dir,category_keyword_dict,category_weight_dict):
    with BertClient(ip='10.0.11.212', port=5555, port_out=5556, show_server_config=True, timeout=100000) as bc:
        ...
        # business_weight=category_weight_dict['business']
        # entertainment_weight = category_weight_dict['entertainment']
        # politics_weight = category_weight_dict['politics']
        # sport_weight = category_weight_dict['sport']
        # tech_weight = category_weight_dict['tech']
        ...
        for i in range(len(test_dir_list)):
            ...
            for j in range(len(second_dir_list)):
                ...
                predictFile_keyword_list,predictFile_weight_list=readFile(path)
                predict_vec=bc.encode(predictFile_keyword_list,is_tokenized=False)
                # print(predict_vec.shape,'   333')
                predict_c=center(predict_vec)
                # print(predict_c.shape, '   555')

                predict_category =categorize_knn_weight(category_all_vec_dict,predict_vec,category_weight_dict,predictFile_weight_list)
                # predict_category=categorize(category_dict,predict_c)
                # predict_num_dict[predict_category] += 1
                y_pred.append(predict_category)
                # if predict_category==right_category:
                #     rightNum+=1
                #     predict_right_dict[right_category]+=1
                # print('right_category    ', right_category, ';  predict_category    ', predict_category)
            # print('cate:      ',test_dir_list[i],'  allNum:   ',allNum,';  rightNum    ',rightNum)
        #     result+='cate:   '
        #     result+=test_dir_list[i]
        #     result+='  allNum:   '
        #     result+=str(allNum)
        #     result+='  rightNum    '
        #     result+=str(rightNum)
        #     result +='\n'
        # print(result,'   !!!')
        # print('tech:   p=',predict_right_dict['tech']/predict_num_dict['tech'],'     r=',predict_right_dict['tech']/original_num_dict['tech'])
        # print('business:   p=',predict_right_dict['business']/predict_num_dict['business'],'     r=',predict_right_dict['business']/original_num_dict['business'])
        # print('entertainment:   p=', predict_right_dict['entertainment'] / predict_num_dict['entertainment'], '     r=',predict_right_dict['entertainment'] / original_num_dict['entertainment'])
        # print('politics:   p=', predict_right_dict['politics'] / predict_num_dict['politics'], '     r=',predict_right_dict['politics'] / original_num_dict['politics'])
        # print('sport:   p=',predict_right_dict['sport']/predict_num_dict['sport'],'     r=',predict_right_dict['sport']/original_num_dict['sport'])
        # original_num=original_num_dict['sport']+original_num_dict['tech']+original_num_dict['politics']+original_num_dict['business']+original_num_dict['entertainment']
        # predict_right_num=predict_right_dict['sport'] + predict_right_dict['entertainment'] + predict_right_dict['business'] + predict_right_dict['tech'] + predict_right_dict['politics']
        # predict_num=predict_num_dict['sport']+predict_num_dict['entertainment']+predict_num_dict['business']+predict_num_dict['politics']+predict_num_dict['tech']
        # print('ALL:   p=',predict_right_num/predict_num,'     r=',predict_right_num/original_num)
        # print('right dict    ',predict_right_dict)
        # print('predict_dict    ',predict_num_dict)
        # print('original dict    ',original_num_dict)
        print('y_true    ',y_true)
        print('y_pred    ',y_pred)
category_keyword_dict,category_weight_dict=readCategoryKeyWord()
test_dir='BBC-test'
classify(test_dir,category_keyword_dict,category_weight_dict)
# test_dir_list=os.listdir(test_dir)
# rightNum=0
# allNum=0
# for i in range(len(test_dir_list)):
